...
export const ResourceScheduler: Component<ResourceSchedulerProps> = (rawProps) => {
  ...
  // --- DERIVED COMPUTATIONS (createMemo) ---
  ...
  const rowVirtualizer = createVirtualizer({
    get count() {
      scrollElement(); // Force reactivity trigger
      return localResources().length;
    },
  ...
  createEffect(() => {
    ...
    if (layout) rowVirtualizer.measure();
  });

  // --- HEADER EVENT HANDLERS ---
  const handleMapViewToggle = () => setIsMapViewActive(!isMapViewActive());
  ...
  const startCardDrag = (e: PointerEvent, eventId: string) => {
    ...
    setInteraction({
      ...
      startScrollLeft: scrollContainerRef?.scrollLeft || 0,
      startScrollTop: scrollContainerRef?.scrollTop || 0,
  ...
  const startRowDrag = (e: PointerEvent, resourceId: string) => {
    ...
    if (idx !== -1) {
      setRowDrag({ resourceId, startY: e.clientY, currentIndex: idx, startScrollTop: scrollContainerRef?.scrollTop || 0 });
      setRowDropIndicator(idx);
    }
  };

  const handlePointerMove = (e: PointerEvent) => {
    if (!gridRef || !scrollContainerRef) return;
    ...
    if (activeMode === 'RESIZING_CARD' && interact) {
      ...
      const scrollDeltaX = scrollContainerRef.scrollLeft - interact.startScrollLeft;
      const scrollDeltaY = scrollContainerRef.scrollTop - interact.startScrollTop;
      ...
      if (props.canChangeRows) {
        const gridY = (e.clientY - metrics.containerTop + scrollContainerRef.scrollTop) - LAYOUT_CONSTANTS.HEADER_OFFSET;
      ...
      const scrollDeltaY = scrollContainerRef.scrollTop - currentDrag.startScrollTop;
  ...
  return (
    <div class="flex flex-col w-full h-full flex-1 bg-background overflow-hidden relative transition-colors duration-200">
      ...
      />
      <div
        ref={(el) => { scrollContainerRef = el; setScrollElement(el); }}
